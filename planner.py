...
def generate_plan(cpf,password,cod,capital):
    ...
    print('AVISO: dados carregados com sucesso. Computando planejamento, aguarde...')
    ...
    for _,row in df.iterrows():
        target_value = round((df['Valor'].sum() + capital) * row['% alvo']/100,2)
        
        if row['Valor'] < target_value:
            quant = (target_value-row['Valor'])//row['Preço'] # se aproxime do valor alvo
    ...
    if not wait_for:
        print('Rebalanceamento concluído com sucesso. Investindo o restante do aporte.')
        ...
        df['Valor planejado'] = (df['Quantidade']+df['Quantidade para comprar']) * df['Preço'] + non_allocated_capital
        ...
        for _,row in df.iterrows():
            ...
            while True:
                quant += 1
                percent = (row['Valor planejado'] + quant * row['Preço'])*100/df['Valor planejado'].sum()
    ...
    for _,row in df[df['Quantidade para comprar']>0].iterrows():
        print('\t{}: {:.0f} X R$ {:.2f}'.format(row['Ticket'],row['Quantidade para comprar'],row['Preço'])) 
